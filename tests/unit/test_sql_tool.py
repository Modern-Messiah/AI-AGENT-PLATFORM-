...
def test_blocked_does_not_match_plain_select(
    plain: str = "SELECT id FROM documents WHERE tenant_id = '{tenant_id}'",
) -> None:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "SELECT * FROM chat_sessions s JOIN chat_messages m ON m.session_id = s.id",
