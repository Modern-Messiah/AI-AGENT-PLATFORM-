...
test('knowledge base file table scrolls inside its card', () => {
  ...
  assert.match(documentsViewSource, /\.documents-memory-card\s*\{[^}]*flex:\s*0 0 clamp\(220px,\s*25vh,\s*280px\)/s)
  assert.match(documentsViewSource, /\.documents-memory-card\s*\{[^}]*min-height:\s*220px/s)
