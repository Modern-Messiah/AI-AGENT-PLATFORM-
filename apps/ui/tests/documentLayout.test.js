...
test('knowledge base file table scrolls inside its card', () => {
  ...
  assert.match(documentsViewSource, /\.documents-memory-card\s*\{[^}]*flex:\s*0 0 clamp\(260px,\s*30vh,\s*340px\)/s)
  assert.match(documentsViewSource, /\.documents-memory-card\s*\{[^}]*min-height:\s*260px/s)
  assert.match(documentsViewSource, /\.documents-memory-card\s*\{[^}]*max-height:\s*340px/s)
