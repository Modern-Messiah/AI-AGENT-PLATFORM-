...
test('knowledge base file table scrolls inside its card', () => {
  ...
  assert.match(documentsViewSource, /\.documents-memory-card\s*\{[^}]*flex:\s*1 1 560px/s)
  assert.match(documentsViewSource, /\.documents-memory-card\s*\{[^}]*min-height:\s*500px/s)
